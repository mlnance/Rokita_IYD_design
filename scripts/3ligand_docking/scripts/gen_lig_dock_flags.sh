...
Example Usage:
./gen_lig_dock_flags.sh bact_IYD_02961
where $1 = bact_IYD_02961 gives:
(1) -in:file:s in_pdb/single_ligand_docking/$1_docking.pdb ligands/2IP.pdb
(2) -parser:protocol 1$1_dock.xml
(3) -out:file:scorefile $1_single_ligand_docking_score.sc
(4) -out:path:pdb out_pdb/single_ligand_docking/$1
...
mkdir -p "out_pdb/single_ligand_docking/$1"
...
-in:file:s 'in_pdb/single_ligand_docking/$1_docking.pdb ligands/2IP.pdb'
...
-out:file:scorefile     $1_single_ligand_docking_score.sc
-out:path:pdb   out_pdb/single_ligand_docking/$1
-out:pdb_gz
